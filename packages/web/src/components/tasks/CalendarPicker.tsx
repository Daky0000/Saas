...
export default function CalendarPicker({ value, onChange, onCancel }: Props) {
  ...
  const pickDay = (d: number) => setSelDate({ y: viewYear, m: viewMonth, d });
    ...
    <div className="rounded-2xl bg-white border border-gray-200 shadow-2xl w-72 select-none">
      ...
      <div className="grid grid-cols-7 px-3 pb-3 gap-y-0.5">
        {cells.map((cell, i) => {
          ...
          const sel   = cell.own && isSelected(cell.day);
          return (
            <button
              key={i}
              ...
              className={[
                ...
                cell.own && !today && !sel          ? 'text-gray-700 hover:bg-indigo-50 hover:text-[#5b6cf9]'   : '',
                today && !sel                       ? 'text-[#5b6cf9] font-bold ring-1 ring-[#5b6cf9]'           : '',
