...
import { Key, CheckCircle2, XCircle, Loader2, Video, Image, Clapperboard } from 'lucide-react';
import { getApiBaseUrl } from '../../utils/apiBase';

const tok = () => localStorage.getItem('auth_token') ?? '';

const KLING_VIDEO_MODELS = [
  { id: 'kling-v2.6-pro',      label: 'Kling v2.6 Pro',      tier: 'Premium', credits: 35 },
  { id: 'kling-v2.5-turbo',    label: 'Kling v2.5 Turbo',    tier: 'Fast',    credits: 20 },
  { id: 'kling-v1.6-pro',      label: 'Kling v1.6 Pro',      tier: 'Quality', credits: 25 },
  { id: 'kling-v1.6-standard', label: 'Kling v1.6 Standard', tier: 'Standard', credits: 15 },
];

const KLING_IMAGE_MODELS = [
  { id: 'kling-v1-5', label: 'Kling v1.5', credits: 5 },
  { id: 'kling-v1',   label: 'Kling v1',   credits: 4 },
];
...
export default function AdminKling() {
  const [ak, setAk]           = useState('');
  const [sk, setSk]           = useState('');
  const [maskedAk, setMaskedAk] = useState('');
  const [maskedSk, setMaskedSk] = useState('');
  ...
  async function fetchConfig() {
    try {
      const r = await fetch(`${getApiBaseUrl()}/api/admin/kling/config`, { headers });
      const d = await r.json();
      if (d.success) { setHasKey(d.hasKey); setMaskedAk(d.maskedAk ?? ''); setMaskedSk(d.maskedSk ?? ''); }
    } catch { /* ignore */ }
  }
  ...
  async function saveKeys() {
    ...
    try {
      const r = await fetch(`${getApiBaseUrl()}/api/admin/kling/config`, {
        method: 'PUT', headers, body: JSON.stringify({ accessKey: ak.trim(), secretKey: sk.trim() }),
      });
      const d = await r.json();
      if (d.success) { setSaveMsg('Saved!'); setAk(''); setSk(''); fetchConfig(); }
      else setSaveMsg(d.error ?? 'Save failed');
    } catch (e: any) { setSaveMsg(e.message); } finally { setSaving(false); }
  }

  async function testConnection() {
    setTesting(true); setTestResult(null); setTestError(''); setAccount(null);
    try {
      const r = await fetch(`${getApiBaseUrl()}/api/admin/kling/test`, { headers });
      const d = await r.json();
      if (d.success) { setTestResult('ok'); setAccount(d.account); }
  ...
  async function fetchGenerations() {
    setLoadingGens(true);
    try {
      const r = await fetch(`${getApiBaseUrl()}/api/admin/kling/generations`, { headers });
      const d = await r.json();
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-4xl font-black tracking-[-0.04em] text-slate-950">Kling AI</h1>
        <p className="mt-2 text-base text-slate-500">
          Native Kling AI integration — text-to-video, image-to-video, and image generation.
          Get your API keys at{' '}
          <a href="https://app.klingai.com/global/dev" target="_blank" rel="noreferrer" className="text-indigo-600 underline">
      ...
      <div className="rounded-[28px] border border-slate-200 bg-white p-6">
        <div className="mb-5 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-sm text-slate-500">Access Key (AK) + Secret Key (SK) from Kling developer console</div>
          </div>
          {hasKey && <span className="ml-auto inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700"><CheckCircle2 size={12} /> Configured</span>}
        </div>

        {hasKey && (
          <div className="mb-4 grid gap-3 sm:grid-cols-2">
            <div className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3">
              <div className="text-xs font-semibold text-slate-400">Access Key</div>
              <div className="mt-1 font-mono text-sm text-slate-700">{maskedAk || '(set via env)'}</div>
            </div>
            <div className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3">
              <div className="text-xs font-semibold text-slate-400">Secret Key</div>
              <div className="mt-1 font-mono text-sm text-slate-700">{maskedSk || '(set via env)'}</div>
            </div>
          </div>
        )}

        <div className="grid gap-3 sm:grid-cols-2">
          <input value={ak} onChange={(e) => setAk(e.target.value)} placeholder="Access Key (AK)"
          ...
          <input value={sk} onChange={(e) => setSk(e.target.value)} placeholder="Secret Key (SK)" type="password"
            className="rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100" />
        </div>

        <div className="mt-4 flex items-center gap-3">
          <button onClick={() => void saveKeys()} disabled={saving || !ak.trim() || !sk.trim()}
          ...
          {saveMsg && <span className={`text-sm font-semibold ${saveMsg === 'Saved!' ? 'text-emerald-600' : 'text-rose-600'}`}>{saveMsg}</span>}
        </div>
        ...
        {testResult && (
          <div className={`mt-4 flex items-start gap-3 rounded-2xl border p-4 ${testResult === 'ok' ? 'border-emerald-200 bg-emerald-50' : 'border-rose-200 bg-rose-50'}`}>
            {testResult === 'ok' ? <CheckCircle2 size={18} className="mt-0.5 text-emerald-600" /> : <XCircle size={18} className="mt-0.5 text-rose-600" />}
            <div>
              {testResult === 'ok'
                ? <><div className="font-semibold text-emerald-700">Connected</div>
                    {account && <pre className="mt-1 text-xs text-emerald-600">{JSON.stringify(account, null, 2)}</pre>}</>
      ...
      <div className="rounded-[28px] border border-slate-200 bg-white">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-5">
          ...
          <button onClick={() => void fetchGenerations()} className="rounded-xl border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-600">
