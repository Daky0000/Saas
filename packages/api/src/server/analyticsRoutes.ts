...
router.get('/api/social/linkedin/company-posts', async (req: Request, res: Response) => {
  ...
  // ─── Frontend-compat analytics adapter routes ────────────────────────────
  ...
  router.get('/analytics/overview', async (req: Request, res: Response) => {
    try {
      ...
      const [postsRes, metricsRes, platformRes, trendsRes, prevMetricsRes] = await Promise.all([
        pool.query(
          `SELECT COUNT(*) as count FROM publishing_logs WHERE user_id=$1 AND status IN ('success','published') AND created_at >= $2`,
          [auth.userId, sinceIso]
        ),
        pool.query(
          `SELECT COALESCE(SUM(reach),0) as total_reach, COALESCE(SUM(impressions),0) as total_impressions, COALESCE(SUM(engagement),0) as total_engagement
           FROM social_metrics WHERE user_id=$1`,
          [auth.userId]
        ),
        pool.query(
          `SELECT platform, COALESCE(SUM(reach),0) as reach, COALESCE(SUM(impressions),0) as impressions, COALESCE(SUM(engagement),0) as engagement
           FROM social_metrics WHERE user_id=$1 GROUP BY platform`,
          [auth.userId]
        ),
        pool.query(
          `SELECT DATE(posted_at) as date, COALESCE(SUM(engagement),0) as total_engagement, COALESCE(SUM(reach),0) as total_reach, COALESCE(SUM(impressions),0) as total_impressions
           FROM social_metrics WHERE user_id=$1 AND posted_at >= $2 GROUP BY DATE(posted_at) ORDER BY date`,
          [auth.userId, sinceIso]
        ),
        pool.query(
          `SELECT COALESCE(SUM(engagement),0) as total_engagement FROM social_metrics WHERE user_id=$1 AND posted_at < $2 AND posted_at >= $3`,
          [auth.userId, sinceIso, prevSinceIso]
        ),
      ]);
      ...
      const platforms = (platformRes.rows as any[]).map((r) => {
        ...
        return {
          ...
          engagementRate: impressions > 0 ? parseFloat((engagement / impressions * 100).toFixed(2)) : null,
        };
      });
      ...
      const topPlatforms = [...platforms]
        ...
        .slice(0, 5)
        .map((p) => ({ platform: p.platform, engagement: p.engagement || 0 }));
  ...
  router.get('/analytics/platforms', async (req: Request, res: Response) => {
    try {
      ...
      const [accountsRes, metricsRes] = await Promise.all([
        pool.query(
          `SELECT platform, account_name, handle FROM social_accounts WHERE user_id=$1 AND connected=true`,
          [auth.userId]
        ),
        pool.query(
          `SELECT platform, COALESCE(SUM(reach),0) as reach, COALESCE(SUM(impressions),0) as impressions, COALESCE(SUM(engagement),0) as engagement, COALESCE(SUM(likes),0) as likes
           FROM social_metrics WHERE user_id=$1 AND (posted_at >= $2 OR posted_at IS NULL) GROUP BY platform`,
          [auth.userId, sinceIso]
        ),
      ]);
      ...
      const platformMetrics = (accountsRes.rows as any[]).map((r) => {
        const m = metricsMap.get(r.platform) || {};
        return {
          platform: r.platform,
          accountName: r.account_name || r.handle || null,
          metrics: {
            reach: parseInt(m.reach || '0') || null,
            impressions: parseInt(m.impressions || '0') || null,
            engagement: parseInt(m.engagement || '0') || null,
            likes: parseInt(m.likes || '0') || null,
          },
        };
      });

      return res.json({ success: true, data: platformMetrics });
    } catch (err) {
  ...
  router.get('/analytics/posts', async (req: Request, res: Response) => {
    try {
      ...
      const result = await pool.query(
        `SELECT sm.platform, sm.platform_post_id, sm.post_id, sm.likes, sm.comments, sm.shares, sm.impressions, sm.reach, sm.engagement, sm.posted_at,
                sa.account_name, sa.handle
         FROM social_metrics sm
         LEFT JOIN social_accounts sa ON sa.id = sm.social_account_id
      ...
      const posts = (result.rows as any[]).map((r) => ({
        id: r.platform_post_id || r.post_id,
        platform: r.platform,
        accountName: r.account_name || r.handle || null,
  ...
  router.get('/analytics/trending', async (req: Request, res: Response) => {
    try {
      ...
      const [trendsRes, topPostsRes, topPlatformsRes] = await Promise.all([
        pool.query(
          `SELECT DATE(posted_at) as date, COALESCE(SUM(engagement),0) as engagement, COALESCE(SUM(reach),0) as reach, COALESCE(SUM(impressions),0) as impressions
           FROM social_metrics WHERE user_id=$1 AND posted_at >= $2 GROUP BY DATE(posted_at) ORDER BY date`,
          [auth.userId, sinceIso]
        ),
        pool.query(
          `SELECT platform, platform_post_id, likes, comments, shares, engagement, posted_at FROM social_metrics
           WHERE user_id=$1 AND posted_at >= $2 ORDER BY engagement DESC NULLS LAST LIMIT 5`,
          [auth.userId, sinceIso]
        ),
        pool.query(
          `SELECT platform, COALESCE(SUM(engagement),0) as engagement FROM social_metrics
           WHERE user_id=$1 AND posted_at >= $2 GROUP BY platform ORDER BY engagement DESC LIMIT 5`,
          [auth.userId, sinceIso]
        ),
      ]);
  ...
  router.get('/analytics/comparison', async (req: Request, res: Response) => {
    try {
      ...
      const [thisRes, lastRes] = await Promise.all([
        pool.query(
          `SELECT COALESCE(SUM(engagement),0) as engagement, COALESCE(SUM(reach),0) as reach, COALESCE(SUM(impressions),0) as impressions, COUNT(*) as posts
           FROM social_metrics WHERE user_id=$1 AND posted_at >= $2`,
          [auth.userId, thisMonthStart]
        ),
        pool.query(
          `SELECT COALESCE(SUM(engagement),0) as engagement, COALESCE(SUM(reach),0) as reach, COALESCE(SUM(impressions),0) as impressions, COUNT(*) as posts
           FROM social_metrics WHERE user_id=$1 AND posted_at >= $2 AND posted_at <= $3`,
          [auth.userId, lastMonthStart, lastMonthEnd]
        ),
      ]);

      const thisMonth = {
        engagement: parseInt(thisRes.rows[0]?.engagement || '0'),
        reach: parseInt(thisRes.rows[0]?.reach || '0'),
        impressions: parseInt(thisRes.rows[0]?.impressions || '0'),
        posts: parseInt(thisRes.rows[0]?.posts || '0'),
      };
      const lastMonth = {
        engagement: parseInt(lastRes.rows[0]?.engagement || '0'),
        reach: parseInt(lastRes.rows[0]?.reach || '0'),
        impressions: parseInt(lastRes.rows[0]?.impressions || '0'),
        posts: parseInt(lastRes.rows[0]?.posts || '0'),
      };
      ...
      return res.json({
        ...
        data: {
          ...
          lastMonth,
          change: {
            engagement: pct(thisMonth.engagement, lastMonth.engagement),
            reach: pct(thisMonth.reach, lastMonth.reach),
            impressions: pct(thisMonth.impressions, lastMonth.impressions),
            posts: pct(thisMonth.posts, lastMonth.posts),
          },
        },
      });
    } catch (err) {
